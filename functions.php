...
function insert_workshop_posts()
{
  ...
  foreach ($workshopTexts as $key => $workshopText) {
    ...
    $workshop_post_id = wp_insert_post(array(
      'post_title' => 'workshop ' . $workshop_date,
      'post_content'   => $post_content_by_day,
      'post_status'    => 'publish',
      'comment_status' => 'closed',
      'ping_status'    => 'closed',
    ...
    if ($workshop_post_id) {
      echo "Workshop {$workshop_post_id} is inserted \n";
      add_post_meta($workshop_post_id, 'workshop_date', date(end($workshopsByDay)->starttime));
    }
  }
}
...
function delete_all_posts($args = array('post_type' => array('sessions', 'workshops'), 'numberposts' => -1))
{
  ...
  foreach ($all_posts as $post) {
    ...
    if ($post->post_type == "sessions") {
      ...
      echo "workshiops $post->ID is deleted ... \n";
    }
  }
}
