...
function insert_workshop_posts()
{
    ...
    foreach ($workshopTexts as $key => $workshopText) {
        $workshopsByDay = $wpdb->get_results($wpdb->prepare("SELECT CONCAT(DATE_FORMAT(starttime,%s),'-',DATE_FORMAT(endtime,%s)) AS time, DATE_FORMAT(starttime, %s) AS workshop_date, starttime, room, building, learnerlevel, session, track, language,presenters,sessionnote,sessdetailslist, apitype, searchtext FROM wp_workshops WHERE searchtext=%s AND apilang=%s ORDER BY starttime", '%H:%i', '%H:%i', '%Y-%m-%d', $workshopText, 'en'));
        ...
        foreach ($workshopsByDay as $workshopByDay) {
            ...
            $workshop_infos   = array(
                ...
                'workshop_infos' => $workshop_table
            );
        }
        var_dump($workshop_table);


        $post_content = normalize_whitespace(load_template_part('content', 'workshop', $workshop_infos));
