...
function insert_workshop_posts()
{
  ...
  foreach ($workshopTexts as $key => $workshopText) {
    $workshopsByDay = $wpdb->get_results($wpdb->prepare("SELECT CONCAT(DATE_FORMAT(starttime,%s),'-',DATE_FORMAT(endtime,%s)) AS time, DATE_FORMAT(starttime, %s) AS workshop_date, starttime, room, building, learnerlevel, language,presenters,sessionnote,sessdetailslist, apitype, searchtext FROM wp_workshops WHERE searchtext=%s AND apilang=%s", '%H:%i', '%H:%i', '%Y-%m-%d', $workshopText, 'en'));
