...
function insert_workshop_posts()
{
  ...
  foreach ($workshopTexts as $key => $workshopText) {
    ...
    foreach ($workshopsByDay as $workshopByDay) {
      ...
      $post_content_by_session = normalize_whitespace(load_template_part('content', 'session', $workshop_infos));
    ...
    var_dump($post_content_by_session);
