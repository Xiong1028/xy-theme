...
function insert_workshop_posts()
{
    ...
    foreach ($workshopsByDay as $key => $workshop) {
        ...
        $post_content = normalize_whitespace(load_template_part('content',  array('workshop_date' => $workshop_date), 'workshop'));
...
function load_template_part($template_name, $workshop_arr, $part_name = null)
{
    ob_start();
    get_template_part($template_name, $workshop_arr, $part_name,);
    $var = ob_get_contents();
    ob_end_clean();
    return $var;
}
